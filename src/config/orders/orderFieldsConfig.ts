...
export const orderFormFields = (): Field<CreateOrderDTO>[] => sortByOrder([
  { name: "customer_id", label: "Cliente", type: "select", validation: { required: true }, order: 0 },
  ...
  // Puedes agregar campos para items/productos aquí
]);

// Campos para formulario de órdenes One-Off
export const orderOneOffFormFields = (): Field<CreateOrderOneOffDTO>[] => sortByOrder([
  { name: "person_id", label: "Cliente", type: "select", validation: { required: true }, order: 0 },
  { name: "sale_channel_id", label: "Canal de Venta", type: "select", validation: { required: true }, order: 1 },
  { name: "price_list_id", label: "Lista de Precios", type: "select", validation: { required: false }, order: 2 },
  { name: "delivery_address", label: "Dirección de Entrega", type: "text", validation: { required: true }, order: 3 },
  { name: "locality_id", label: "Localidad", type: "select", validation: { required: true }, order: 4 },
  { name: "zone_id", label: "Zona", type: "select", validation: { required: true }, order: 5 },
  { name: "purchase_date", label: "Fecha de Compra", type: "date", validation: { required: true }, order: 6 },
  { name: "notes", label: "Notas", type: "text", validation: { required: false }, order: 7 },
  { name: "payment_method_id", label: "Método de Pago", type: "select", validation: { required: false }, order: 8 },
  // Puedes agregar campos para items/productos aquí
]);
